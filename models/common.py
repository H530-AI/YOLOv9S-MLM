...
class MCA(nn.Module):
    ...
    def __init__(self, channels, reduction=16):
        ...
        self.fc = nn.Sequential(
            nn.Linear(channels*21, channels//reduction, bias=False),
    ...
    def forward(self, x):
        ...
        y1 = self.avg_pool1(x).view(b, c)          # [b, c]
        y2 = self.avg_pool2(x).view(b, c*4)        # [b, c*4]
        y4 = self.avg_pool4(x).view(b, c*16)       # [b, c*16]
        
        # Concatenate and process
        y = torch.cat((y1, y2, y4), 1)             # [b, c*21]
